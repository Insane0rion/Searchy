...
class FileHandler:
    ...
    @property
    def _setRoot(self):
        splitted_path = realpath(__file__).split("/")
        for _ in range(2):
            del splitted_path[-1]
        return "/".join(splitted_path) + "/"

    def getSettings(self) -> dict | bool:
        if self.prove_settings_file():
    ...
    def prove_settings_file(self) -> bool:
        ...
        try:
            ...
            input(
                "Settings.ini has been created! If you want to use the Youtube engine please insert you're API-Key!"
            )
            return True
        except:
            return False
